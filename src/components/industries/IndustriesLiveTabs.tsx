...
export function IndustriesLiveTabs() {
  ...
  const syncFromScroll = useCallback(() => {
    ...
    const first = el.querySelector<HTMLElement>("[data-industry-slide]");
    if (!first) return;
  ...
  const scrollToIndex = useCallback((index: number) => {
    const el = scrollerRef.current;
    if (!el) return;
    const slides = el.querySelectorAll<HTMLElement>("[data-industry-slide]");
          ...
          {tabs.map((industry, i) => {
            ...
            return (
              <button
                ...
                aria-label={zh ? industry.titleZh : industry.title}
